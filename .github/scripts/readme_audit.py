...
PLACEHOLDERS = [
    ...
    "one-sentence category and value description",
    "short project description",
    "name the primary users",
    "name the concrete pain",
    "name the useful result",
    "state the maturity level",
    "add screenshot",
    "add terminal output",
...
REQUIRED_SIGNALS = [
    ...
    ("category", re.compile(r"\b(library|cli|command line|app|application|framework|template|integration|plugin|toolkit|dataset|model|service|api|sdk|workflow|github action|terraform|helm|chart)\b", re.I), "name the project category"),
    ("audience", re.compile(r"\b(who it'?s for|who is it for|audience|primary users|for developers|for maintainers|for teams|for operators|for contributors|for users|intended users|built for)\b", re.I), "name the intended audience"),
    ...
    ("outcome", re.compile(r"\b(outcome|result|so you can|helps|lets you|enables|reduces|prevents|automates|improves|faster|safer|clearer)\b", re.I), "state the user outcome or benefit"),
    ("fit_guidance", re.compile(r"\b(use this|good fit|not a fit|not ideal|when to use|when not to use|alternatives|compare|comparison)\b", re.I), "document when to use it and when not to use it"),
...
FIRST_SCREEN_SIGNALS = [
    ("purpose", re.compile(r"\b(does|is a|is an|overview|purpose|about)\b", re.I)),
    ("audience", re.compile(r"\b(for developers|for maintainers|for teams|for operators|for users|who it'?s for|who is it for|primary users|built for)\b", re.I)),
    ("problem", re.compile(r"\b(problem|solves|pain|workflow|use case|why)\b", re.I)),
    ("outcome", re.compile(r"\b(outcome|result|helps|lets you|so you can|reduces|automates|prevents|safer|faster|clearer)\b", re.I)),
]
...
def first_screen_missing(text: str) -> list[str]:
    first_screen = text[:1600]
    return [name for name, pattern in FIRST_SCREEN_SIGNALS if not pattern.search(first_screen)]
...
def audit(root: Path) -> dict[str, object]:
    ...
    missing_first_screen = first_screen_missing(text)
    if missing_first_screen:
        findings.append(Finding("warn", "first_screen_clarity", rel, "early README should include: " + ", ".join(missing_first_screen)))
    else:
        findings.append(Finding("ok", "first_screen_clarity", rel, "purpose, audience, problem, and outcome appear early"))
